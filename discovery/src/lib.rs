//! Сервис/клиент для обнаружения серверов в сети
//! 
//! Основная задача - сделать рассылку по адресам и подождать какие сервера ответят
//! Ответ сервера должен содержать адрес регистрации 
