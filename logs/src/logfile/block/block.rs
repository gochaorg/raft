...
impl Block {
        ...
        Source: ReadBytesFrom,
    {
        ...
        Ok((
            ...
            position + (head_size.0 as u64) + (data_size.0 as u64) + (tail_size.0 as u64),
        ))
    }
    ...
    fn to_bytes(
        ...
        bbuf: &mut ByteBuff,
        tracker: &Tracker,
    ) -> (BlockHeadSize, BlockDataSize, BlockTailSize) {
        // write tail marker
        let t0 = Instant::now();
        ...
        tracker.track("to_bytes/write_block_head", || {
            self.head
                .write_block_head(bbuf, self.data.len() as u32, tail.len() as u16, tracker)
        });

        let block_head_size = tracker.track("to_bytes/block_head_size", || {
            BlockHeadSize(bbuf.buff.len() as u32)
        });
        let block_data_size = tracker.track("to_bytes/block_data_size", || {
            BlockDataSize(self.data.len() as u32)
        });
        let block_tail_size = tracker.track("to_bytes/block_tail_size", || {
            BlockTailSize(tail.len() as u16)
        });

        // copy data
        tracker.track("to_bytes/copy data", || bbuf.write_byte_arr(&self.data));
    ...
    pub fn write_to<Destination>(
        ...
        block_buff: &mut ByteBuff,
        tracker: &Tracker,
    ) -> Result<BlockHeadRead, BlockErr>
    where
        Destination: WriteBytesTo,
    {
...
fn test_block_rw() {
    ...
    block
        .write_to(0, &mut bb, &mut block_buff, &tracker)
        .unwrap();
