...
impl Tail {
    ...
    pub fn try_read_block_at<Source>(
        position: u64,
        source: &Source,
    ) -> Result<(Block, u64), BlockErr>
    where
        Source: ReadBytesFrom,
    {
        if position < (TAIL_SIZE as u64) {
            return Err(BlockErr::tail_position_to_small(TAIL_SIZE, position));
        }
        ...
        if reads < tail_data.len() as u64 {
            return Err(BlockErr::no_data(reads as u64, TAIL_SIZE as u64));
        }

        let marker_matched = (0usize..4usize).fold(true, |res, idx| {
            res && TAIL_MARKER.as_bytes()[idx] == tail_data[idx]
        });
        if !marker_matched {
            return Err(BlockErr::TailMarkerMismatched {
                tail_data: tail_data,
            });
        }
        ...
        if next_pos < 0 {
            return Err(BlockErr::TailPointerOuside { pointer: next_pos });
        }
    ...
    /// Чтение заголовка
    pub fn try_read_head_at<Source>(
        position: u64,
        source: &Source,
        ...
        Source: ReadBytesFrom,
    {
        ...
        let reads = source.read_from((position) - (TAIL_SIZE as u64), &mut tail_data)?;
        if reads < tail_data.len() as u64 {
            return Err(BlockErr::no_data(reads, TAIL_SIZE as u64));
        }
